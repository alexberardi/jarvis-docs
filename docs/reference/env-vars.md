...
|----------|---------|--------------|
| `DATABASE_URL` | auth, recipes, config-service, notifications | PostgreSQL connection string (e.g., `postgresql://user:pass@host:5432/dbname`). Command-center uses `DB_URL` for the same purpose. |
| `AUTH_SECRET_KEY` | auth + every service that validates JWTs | JWT signing key (HS256). Must be the same across all services that validate JWTs. |
