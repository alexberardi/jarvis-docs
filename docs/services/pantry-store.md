...
Hashless lockfiles (`pip-compile` without `--generate-hashes`) are used since the hash-per-artifact format inflates lockfile size dramatically for packages with many wheel variants (e.g. `aiohttp` alone contributes ~50 hash lines). Version pinning is preserved; only the per-wheel SHA256 hashes are omitted.

**Backward compatibility**: `lockfile_to_package_specs` tolerates `--hash=sha256:` continuation lines in existing stored lockfiles, so previously submitted hashed lockfiles still parse correctly when the runner consumes them.

**Trust model**: Lockfile entries remain version-pinned at submission time. The security boundary is PyPI + TLS + the submission-acceptance resolve step — the same default trust model used by most Python projects.
