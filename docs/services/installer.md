...
### Auth Refresh-Token Grace Window (`REFRESH_TOKEN_GRACE_SECONDS`)

Since jarvis-installer#4 (part of roadmap#44), the `jarvis-auth` service entry in `service-registry.json` declares `REFRESH_TOKEN_GRACE_SECONDS` (`required: false`, `default: "10"`) alongside the service's other env vars, giving installer users a documented install-time path to set the refresh-token grace window (see [Auth: Token Rotation](auth.md#token-rotation)) instead of relying on jarvis-auth's own code default. Under live DB-tunability, this still matters as the install-time fallback: `env_fallback` honors it as the default before any admin-set DB override takes effect. No generator changes were needed — the var is optional with a default, so compose/env emission is unaffected either way.

### Supervised llm-proxy Launch (serve.sh)

Since jarvis-installer#21, the exported `llm-proxy` service's `command` is `["bash", "scripts/serve.sh"]` (the supervised launcher the image ships, added in jarvis-llm-proxy-api#21) instead of a raw unsupervised dual-uvicorn shell command. `serve.sh` runs the API server in the foreground and supervises the model service, respawning it with backoff on a native crash (e.g. a llama.cpp segfault) — previously a crashed model service was never restarted and the API 503'd every request indefinitely while the container itself still looked healthy (the 2026-07-02 outage class, roadmap#59). Only `compose-export-generator.ts` emitted the old pattern — `compose-generator.ts` (interactive preview) relies on the image's own `CMD` for llm-proxy and was unaffected. This is the installer-side twin of the same fix in jarvis-admin's own generator (see [Admin: Supervised llm-proxy Launch](admin.md#supervised-llm-proxy-launch-servesh)).
