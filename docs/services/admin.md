...
Since jarvis-admin#36, the sync/reconcile flow persists `WHISPER_BACKEND` and the new `TTS_BACKEND` (+ `TTS_GPU_DEVICE`) into `.env` and reads them back on regenerate. Previously the state-reconstructor recomputed these from scratch on every reconcile, so an existing install's GPU-backed Whisper or TTS silently reverted to `cpu` on the next "Sync Compose" (prod incidents 2026-07-04 and 2026-07-06: Whisper CUDA→CPU dropped STT from ~90ms to ~16s; TTS lost its GPU device passthrough entirely). ReconcilePage now hydrates both the Whisper backend selector and a new TTS **Inference device** selector from `GET /api/install/options` before rendering the form, so a plain "Sync Compose" preserves whatever backend was already running.

TTS device selection (`TtsBackend`: `cpu` | `cuda`) only controls GPU device passthrough — the stock `jarvis-tts` image ships CUDA-capable torch already, so unlike Whisper there is no separate `-cuda` image variant. A single GPU is pinned via `TTS_GPU_DEVICE` (default `0`); the generator never emits `count: all`, since Kokoro's warmup can OOM next to an already-loaded LLM sharing the same card.
...
| `WHISPER_BACKEND` | Whisper image variant / device passthrough selected via the setup wizard or reconcile flow (`cpu`, `cuda`, `vulkan`, `rocm`; default `cpu`). Persisted in `.env` since jarvis-admin#36 so reconcile no longer silently downgrades a GPU install — see [GPU Backend Persistence](#gpu-backend-persistence-reconcile). |
| `TTS_BACKEND` | TTS (Kokoro) inference device selected via the reconcile flow (`cpu` or `cuda`, default `cpu`). Device passthrough only — no image variant. Added jarvis-admin#36. |
| `TTS_GPU_DEVICE` | GPU index for TTS device passthrough when `TTS_BACKEND=cuda` (default `0`). Added jarvis-admin#36. |
