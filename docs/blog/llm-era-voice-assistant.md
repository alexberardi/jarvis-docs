...
**A package store for voice capabilities.** **20 commands ship built in** — conversation, memories, timers, reminders, routines, web search, smart-home control, node administration. Another **24 packages** are one click away in the [Pantry](https://pantry.jarvisautomation.io): weather, news, sports, music (Spotify, Pandora, Music Assistant), calendar, email, movies, and the device integrations above. Each is a standalone repo you can fork, modify, or replace. Adding your own means implementing one Python interface.
